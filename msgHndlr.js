...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            ...
            error: {
                St: '[❗] Envie a imagem com a legenda *!sticker* ou marque a imagem que foi enviada',
                ...
                Ki: '[❗] O bot não pode emitir um administrador de grupo!',
        ...
        case '!sgif':
            if (isMedia) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    const mediaData = await decryptMedia(message, uaOverride)
                    client.reply(from, '[AGUARDE] Em andamento⏳ aguarde ± 1 min!', id)
                    ...
                    client.reply(from, '[❗] Enviar vídeo com a legenda *!StickerGif * máx. 10 seg!', id)
                )
            }
            break
	    case '!stickernobg':
        ...
        case '!ytmp4':
            ...
            try {
                ...
                if (ytv.error) {
                    ...
                    if (Number(ytv.filesize.split(' MB')[0]) > 40.00) return client.reply(from, 'Envie o comando *!Ytmp4 [linkYt] *, por exemplo, envie o comando *!Readme *', id)
            ...
            break
        case '!wiki':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!Wiki [consulta] *\ nExemplo: *!Wiki asu *', id)
            ...
            break
        case '!cuaca':
            if (args.length === 1) return client.reply(from,  'Enviar comando *! Clima [local] * \ nExemplo: *! Tempo tangerang Enviar comando *! Tempo [local] * \ nExemplo: *! Tempo tangerang', id)
        ...
        case '!fb':
            ...
            break
        case '!creator':
            client.sendContact(from, '5531985642195@c.us')
            break
        case '!ig':
        ...
        case '!infogempa':
            ...
            break
        case '!anime':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!Anime [query] * \ nExemplo: *!Anime Darling in the franxx*', id)
        ...
        case '!nh':
            ...
            if (args.length === 2) {
                ...
                client.reply(from, '[ERRADO] Enviar comando *!Nh [nuClear] * por exemplo, enviar comando *!Readme')
            }
        	break
        case '!brainly':
            if (args.length >= 2){
        ...
        case '!quotemaker':
            ...
            if (arg.length >= 4) {
                ...
                await quotemaker(quotes, author, theme).then(amsu => {
                    client.sendFile(from, amsu, 'quotesmaker.jpg','eh...').catch(() => {
        ...
        case '!linkgroup':
            ...
            break
        case '!bc':
            if (!isOwner) return client.reply(from, 'Este pedido é apenas para proprietários de bot!', id)
        ...
        case '!tagall':
            ...
            let hehe = '╔══✪〘 Mencionando Todos 〙✪══\n'
            for (let i = 0; i < groupMem.length; i++) {
                hehe += '╠➥'
                hehe += ` @${groupMem[i].id.replace(/@c.us/g, '')}\n`
            }
            hehe += '╚═〘 ⁖℘༒𝕿𝖔𝖇𝖎𝖗𝖆𝖒𝖆 𝕾𝖊𝖓𝖏𝖚—͟͟͞͞𖣘 BOT 〙'
        ...
        case '!kickall':
            ...
            client.reply(from, 'Sucesso banir todos os membros', id)
            break
        case '!leaveall':
            if (!isOwner) return client.reply(from, 'Este pedido é apenas para proprietários de bot!', id)
            ...
            client.reply(from, 'Sucesso tirar todo o grupo!', id)
            break
        case '!clearall':
            if (!isOwner) return client.reply(from, 'Este pedido é apenas para proprietários de bot!', id)
            ...
            client.reply(from, 'Sucesso ao limpar todo o chat!', id)
            break
        case '!add':
            const orang = args[1]
        ...
        case '!kick':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado pelos administradores do grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este comando só pode ser usado quando o bot é um administrador', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este comando, envie o comando *! Kick * @tagmember', id)
        ...
        case '!leave':
            if (!isGroupMsg) return client.reply(from, 'Este comando só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este comando só pode ser usado por administradores de grupo', id)
            await client.sendText(from,'Sayonara Pessoal👋').then(() => client.leaveGroup(groupId))
            break
        case '!promote':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
        ...
        case '!demote':
            ...
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores de grupo', id)
            if (!isBotGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado quando o bot é um administrador', id)
            if (mentionedJidList.length === 0) return client.reply(from, 'Para usar este recurso, envie o comando *!demote* @tagadmin', id)
        ...
        case '!join':
            ...
            if (args.length < 2) return client.reply(from, 'Envie o comando *!Join à chave do grupo de links* \ n \ nEx: \ n !join a https://chat.whatsapp.com/blablablablablabla abcde \ npara obter a chave, você só pode obtê-la com uma doação de 5k', id)
            const link = args[1]
            const key = args[2]
            const tGr = await client.getAllGroups()
            const minMem = 30
            const isLink = link.match(/(https:\/\/chat.whatsapp.com)/gi)
            if (key !== 'lGjYt4zA5SQlTDx9z9Ca') return client.reply(from, '*key* errado! converse com o proprietário do bot para obter uma chave válida', id)
            ...
            if (check.status === 200) {
                await client.joinGroupViaLink(link).then(() => client.reply(from, 'O barco chegará em breve!'))
            } else {
        ...
        case '!del':
            if (!isGroupMsg) return client.reply(from, 'Este recurso só pode ser usado em grupos', id)
            if (!isGroupAdmins) return client.reply(from, 'Este recurso só pode ser usado por administradores de grupo', id)
            if (!quotedMsg) return client.reply(from, 'Errado !!, envie o comando *!delete ou !del [somente mensagem do bot]*', id)
        ...
        case '!getses':
            ...
            client.sendFile(from, sesPic, 'session.png', 'Eh...', id)
            break
        case '!lirik':
            if (args.length == 1) return client.reply(from, 'Send command *!Lirik [opcional] *, example *!Lirik I'm not a doll*', id)
            const lagu = body.slice(7)
            const lirik = await liriklagu(lagu)
            client.reply(from, lirik, id)
            break
        case '!chord':
            if (args.length === 1) return client.reply(from, 'Enviar comando *!Chord [query] *, por exemplo *!chord sans*', id)
        ...
        case '!listblock':
            ...
            break
        case '!jadwalshalat':
            if (args.length === 1) return client.reply(from, '[❗] Enviar comando *!jadwalshalat [área] * \ nexemplo: *!jadwalshalat de Tangerang * \ nPara listar regiões, envie o comando *!ListDaerah*')
        ...
        case '!listchannel':
            ...
            break
        case '!jadwaltv':
            if (args.length === 1) return client.reply(from ('Enviar comando *!jadwalTv [channel]', id)
        ...
        case '!loli':
            ...
            client.sendFileFromUrl(from, loli.result, 'loli.jpeg', 'Lolizinha ^^', id)
            break
        case '!waifu':
        ...
        case '!url2img':
            ...
            if (args.length === 1) return client.reply(from, 'Enviar comando *!Url2img [web] * \ nExemplo *!Url2img https://google.com*', id)
